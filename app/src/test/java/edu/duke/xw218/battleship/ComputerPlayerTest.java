...
public class ComputerPlayerTest {
  ...
  public void test_result() throws IOException {
    BattleShipBoard<Character> b = new BattleShipBoard<Character>(2, 1, 'X');
    BoardTextView e_view = new BoardTextView(b);
    ByteArrayOutputStream bytes = new ByteArrayOutputStream();
    PrintStream output = new PrintStream(bytes, false);
    Board<Character> board = new BattleShipBoard<Character>(7, 8, 'X');
    ...
    b.tryAddShip(ship);
    //    System.out.println(e_view.displayMyOwnBoard());
   
    int flag = 1;
    while(flag == 1){
      if(player.playOneTurn(b, e_view)==false){
        flag = 0;
      }else{
        continue;
      }
